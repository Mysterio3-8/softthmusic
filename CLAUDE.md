...
| Интервал | случайный 160–235 мин | `soundcloud.*_interval_minutes` |
| Предохранитель | 7 постов/сутки | `soundcloud.max_posts_per_day` |
| Возобновление утром | +0..45 мин джиттера (не работает без паузы) | `album_scheduler` |

**Арифметика режима.** 1440 мин / 7 постов = 205 мин — это ПОТОЛОК среднего интервала.
160–235 даёт среднее ~198 и ~7.3 поста, поэтому ограничителем работает суточный лимит,
а интервал отвечает за разброс. Хотите интервал больше — уменьшайте число постов, иначе
никак: 10 постов в сутки уже требуют среднего 144 мин.
