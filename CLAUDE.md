...
**Статус:** 🟢 альбомный поток задеплоен на VPS 2026-07-29, таймер активен, очередь пуста.
YouTube-поток выключен (`yt-vk-publisher.timer` disabled). 63 теста зелёные. Каталог на VPS: `/opt/yt-vk-publisher`, в реестре бота — `p_minus`.
...
```bash
venv/Scripts/python.exe -m pytest tests/ -q       # 63 теста
