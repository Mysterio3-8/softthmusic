...
COOLDOWN_MINUTES_AFTER_ERROR = 10
"""Пауза после ОБЫЧНОЙ ошибки VK.

Было 90 минут — и это оказалось главной причиной, почему за сутки пул отдавал 20 слотов,
а публикаций выходило шесть. Разбор 07.08: у Треков загрузка упала с `[22] Getting
upload URL error`, аккаунт ушёл в кулдаун на полтора часа — а аккаунт ОСТАЛСЯ ОДИН,
поэтому встали разом все четыре софта. Одна случайная ошибка VK стоила полутора часов
тишины во всех группах; за сутки таких эпизодов набиралось несколько.

90 минут имели смысл при пуле из нескольких аккаунтов: подозрительный охлаждаем, грузим
следующим. С одним аккаунтом это просто глобальный стоп. Настоящую защиту от долбёжки
и так даёт MIN_GAP_MINUTES (40 мин) — он всё равно больше этой паузы, так что после
транзиентной ошибки следующая попытка придёт по обычному зазору."""

COOLDOWN_MINUTES_AFTER_BLOCK = 24 * 60
"""`5 / user is blocked` — это бан, а не троттлинг: убираем аккаунт из ротации на сутки.
Настоящие признаки бана (коды 5/17/29) по-прежнему стоят суток — их занижать нельзя."""
